...
router.post("/", function(req, res) {
  ...
  /*
    ... : 전개연산자, spread op
          req.body에 저장된 json 데이터 {todo : "데이터", to_date : "날짜", to_time : "시간"}을 구조까지 똑같이 todo_data로 복제
  */
  let todo_data={...req.body}

  // res.send(todo);
  // res.json(req.body);
  // JSON.strinify(jsonObject) : jsonObject(JSON객체) 데이터를 네트워크를 통해서 전송하기 위하여 문자열로 변환
  // string serializing : 일련화, 나열화
  res.render("index", {todo_data : JSON.stringify(todo_data)});
  // res.render("index", {todo_data : req.body});
});

// get("/insert") : 입력을 받기 위해서 사용자(client)에게 입력 form을 보여주는 용도의 URL
router.get("/insert", function(req, res) {
  res.render("write");
});

// post("/insert") : form에서 입력 받은 내용을 서버로 전송했을 때 데이터를 수신하여 처리하는 용도의 URL
